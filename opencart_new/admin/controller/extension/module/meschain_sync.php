...
class MeschainSync extends \Opencart\System\Engine\Controller {
    ...
    public function testConnection(): void {
        ...
        // Skip session and permission checks for API testing
        // This allows direct API access without login

        $marketplace = isset($_REQUEST['marketplace']) ? $_REQUEST['marketplace'] :
                      (isset($_GET['marketplace']) ? $_GET['marketplace'] :
                      (isset($_POST['marketplace']) ? $_POST['marketplace'] : ''));
        ...
        try {
            ...
            if ($marketplace === 'trendyol') {
                ...
                $this->response->setOutput(json_encode([
                    'success' => false,
                    'marketplace' => $marketplace,
                    'error' => 'Marketplace not supported yet: ' . $marketplace
                ]));
            }
        } catch (Exception $e) {
            $this->response->setOutput(json_encode([
