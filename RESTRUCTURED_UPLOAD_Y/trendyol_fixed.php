...
class Trendyol {
    ...
    public function testConnection(): array {
        ...
        try {
            // First try brands endpoint (known to work)
            $brandsUrl = 'https://api.trendyol.com/sapigw/brands';
            $response = $this->makeDirectRequest('GET', $brandsUrl);
            ...
            if ($response && isset($response['content'])) {
                return [
                    ...
                    'test_endpoint' => 'brands',
                    'brands_count' => count($response['content'])
                ];
            }
            
            // Fallback: try categories endpoint
            $categoriesUrl = 'https://api.trendyol.com/sapigw/product-categories';
            $response = $this->makeDirectRequest('GET', $categoriesUrl);
            
            if ($response && isset($response['content'])) {
                return [
                    'status' => 'success',
                    'message' => 'Trendyol API connection successful (via categories)',
                    'api_version' => 'v1',
                    'supplier_id' => $this->supplier_id,
                    'response_time' => $responseTime . 'ms',
                    'test_endpoint' => 'categories',
                    'categories_count' => count($response['content'])
                ];
            }
            
            throw new \Exception('No valid response from any test endpoint');
            
        } catch (\Exception $e) {
            $endTime = microtime(true);
            $responseTime = round(($endTime - $startTime) * 1000, 2);
            
            throw new \Exception('Trendyol API connection failed: ' . $e->getMessage() . ' (Response time: ' . $responseTime . 'ms)', $e->getCode());
        }
    }
    
    /**
     * Make direct API request to any URL
     */
    private function makeDirectRequest($method, $url, $params = array(), $data = null) {
