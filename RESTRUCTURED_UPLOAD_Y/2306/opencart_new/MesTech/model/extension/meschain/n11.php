...
class N11 extends Model {
    ...
    public function install() {
        ...
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "meschain_n11_orders` (
                ...
                `n11_id` varchar(255) NOT NULL,
                `n11_order_number` varchar(255) NOT NULL,
        ...
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "meschain_n11_sync_log` (
    ...
    public function uninstall() {
        ...
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "meschain_n11_sync_log`");
    }
    ...
    public function getSyncStatistics() {
        ...
        $query = $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "meschain_n11_products` WHERE status = 1");
        ...
        $query = $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "meschain_n11_orders` WHERE DATE(date_added) = CURDATE()");
        $data['today_orders'] = $query->row['total'];
        
        // Last sync time
        $query = $this->db->query("SELECT MAX(last_sync) as last_sync FROM `" . DB_PREFIX . "meschain_n11_products`");
        $data['last_product_sync'] = $query->row['last_sync'];
        
        $query = $this->db->query("SELECT MAX(last_sync) as last_sync FROM `" . DB_PREFIX . "meschain_n11_orders`");
        $data['last_order_sync'] = $query->row['last_sync'];
        
        return $data;
    }
    
    public function syncProductsToN11() {
        try {
            ...
            foreach ($products as $product) {
                ...
                $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "meschain_n11_products` WHERE opencart_product_id = '" . (int)$product['product_id'] . "'");
                
                if ($query->num_rows) {
                    // Update existing product
                    $this->updateProductToN11($product);
                } else {
                    // Create new product
                    $this->createProductToN11($product);
                }
    ...
    public function syncOrdersFromN11() {
        try {
            // N11 API ile sipariş çekme logic'i burada olacak
            $synced_count = 0;
            
            // Placeholder for N11 API integration
            // $orders = $this->n11_api->getOrders();
    ...
    private function createProductToN11($product) {
        // N11 API product creation logic
        $n11_id = 'N11_' . $product['product_id'] . '_' . time();
        
        $this->db->query("
            INSERT INTO `" . DB_PREFIX . "meschain_n11_products` 
            SET opencart_product_id = '" . (int)$product['product_id'] . "',
                n11_id = '" . $this->db->escape($n11_id) . "',
                n11_sku = '" . $this->db->escape($product['sku']) . "',
                status = 1,
                last_sync = NOW(),
                date_added = NOW(),
                date_modified = NOW()
    ...
    private function updateProductToN11($product) {
        // N11 API product update logic
        $this->db->query("
            UPDATE `" . DB_PREFIX . "meschain_n11_products` 
            SET last_sync = NOW(),
                date_modified = NOW()
    ...
    public function getN11Products($data = array()) {
        $sql = "SELECT np.*, p.name, p.sku, p.price, p.status as product_status 
                FROM `" . DB_PREFIX . "meschain_n11_products` np
                LEFT JOIN `" . DB_PREFIX . "product` p ON np.opencart_product_id = p.product_id
        ...
        if (!empty($data['filter_status'])) {
            $sql .= " WHERE np.status = '" . (int)$data['filter_status'] . "'";
        }
    ...
    public function getTotalN11Products() {
        $query = $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "meschain_n11_products`");
        
        return $query->row['total'];
    }
    
    public function deleteN11Product($n11_product_id) {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "meschain_n11_products` WHERE n11_product_id = '" . (int)$n11_product_id . "'");
        
        $this->log('product', 'delete', $n11_product_id, 'Product deleted successfully', 'success');
    }
    
    private function log($type, $action, $entity_id, $message, $status) {
        $this->db->query("
            INSERT INTO `" . DB_PREFIX . "meschain_n11_sync_log` 
    ...
    public function getSyncLogs($data = array()) {
        $sql = "SELECT * FROM `" . DB_PREFIX . "meschain_n11_sync_log`";
