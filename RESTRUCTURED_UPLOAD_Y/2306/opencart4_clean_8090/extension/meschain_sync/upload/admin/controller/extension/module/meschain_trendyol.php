...
class MeschainTrendyol extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_extension_module_meschain_trendyol->saveSettings($this->request->post);
        ...
        $this->response->setOutput($this->load->view('extension/module/meschain/marketplace/trendyol', $data));
    }
    ...
    public function syncProducts(): void {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/meschain_trendyol')) {
            ...
            try {
                $result = $this->model_extension_module_meschain_trendyol->syncProducts();
    ...
    public function getOrders(): void {
        ...
        if (!$this->user->hasPermission('access', 'extension/module/meschain_trendyol')) {
            ...
            try {
                ...
                $orders = $this->model_extension_module_meschain_trendyol->getOrders($filter_data);
    ...
    public function updateInventory(): void {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/meschain_trendyol')) {
            ...
            try {
                ...
                $result = $this->model_extension_module_meschain_trendyol->updateInventory($product_id, $quantity);
    ...
    public function getAnalytics(): void {
        ...
        if (!$this->user->hasPermission('access', 'extension/module/meschain_trendyol')) {
            ...
            try {
                ...
                $analytics = $this->model_extension_module_meschain_trendyol->getAnalytics($period);
    ...
    public function getCampaigns(): void {
        ...
        try {
            $campaigns = $this->model_extension_module_meschain_trendyol->getCampaigns();
    ...
    private function getFormData(): array {
        // Load cron language file
        $this->load->language('extension/meschain/module/meschain_trendyol_cron');
        ...
        // Get saved settings
        $settings = $this->model_extension_module_meschain_trendyol->getSettings();
        ...
        // Cron settings
        $data['module_meschain_trendyol_cron_token'] = $settings['module_meschain_trendyol_cron_token'] ?? '';
        
        // Generate cron URL
        $server = (!empty($this->request->server['HTTPS']) ? 'https://' : 'http://') . $this->request->server['HTTP_HOST'];
        $data['cron_url'] = $server . '/index.php?route=extension/meschain/cron/trendyol';
        
        if ($data['module_meschain_trendyol_cron_token']) {
            $data['cron_url'] .= '&token=' . $data['module_meschain_trendyol_cron_token'];
        }
        
        // Get last cron run
        $query = $this->db->query("
            SELECT created_at 
            FROM `" . DB_PREFIX . "trendyol_event_log` 
            WHERE event_type = 'cron_execution'
            ORDER BY created_at DESC
            LIMIT 1
        ");
        
        $data['cron_last_run'] = $query->num_rows ? $query->row['created_at'] : $this->language->get('text_no_cron_runs');
     ...
     */
    public function getCronLastRun(): void {
        $json = [];
        ...
        if (!$this->user->hasPermission('access', 'extension/module/meschain_trendyol')) {
            ...
            $json['success'] = true;
            $json['last_run'] = $query->num_rows ? $query->row['created_at'] : $this->language->get('text_no_cron_runs');
        }
