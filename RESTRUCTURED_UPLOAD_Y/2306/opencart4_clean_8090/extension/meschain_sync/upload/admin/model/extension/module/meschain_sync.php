...
class MeschainSync extends \Opencart\System\Engine\Model {
    ...
    public function getSystemStatus(): array {
        ...
        try {
            ...
            $db_check = $this->db->query("SELECT 1");
            if ($db_check) {
            ...
            $settings_check = $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "meschain_settings`");
            if ($settings_check->row['total'] > 0) {
                 $status['health']['Config'] = 'healthy';
            } else {
                $status['health']['Config'] = 'warning';
                $status['system'] = 'warning';
            }

            // Check Logs Table
            $logs_check = $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "meschain_logs` WHERE `log_level` = 'error' AND `timestamp` > DATE_SUB(NOW(), INTERVAL 24 HOUR)");
            $error_count = $logs_check->row['total'];
            
            if ($error_count == 0) {
                 $status['health']['Logs'] = 'healthy';
            } elseif ($error_count < 10) {
                $status['health']['Logs'] = 'warning';
                 $status['system'] = 'warning';
            } else {
                $status['health']['Logs'] = 'error';
                $status['system'] = 'error';
            }
            
            // Check Cron/Sync Status (by looking at product sync table)
            $sync_check = $this->db->query("SELECT MAX(last_sync) as last_sync_time FROM `" . DB_PREFIX . "meschain_product_sync`");
            if ($sync_check->row['last_sync_time']) {
                $last_sync = new \DateTime($sync_check->row['last_sync_time']);
                $now = new \DateTime();
                $diff = $now->getTimestamp() - $last_sync->getTimestamp();
                
                if ($diff < 3600) { // Less than 1 hour
                    $status['health']['Sync'] = 'healthy';
                } else {
                    $status['health']['Sync'] = 'warning';
                    $status['system'] = 'warning';
                }
            } else {
                $status['health']['Sync'] = 'warning'; // No sync has run
            }

        } catch (Exception $e) {
            $status['system'] = 'critical_error';
            $status['health']['System'] = 'unhealthy';
