...
class MesChainAuth {
    ...
    // 🧪 AUTH SYSTEM TEST FUNCTIONS
    
    runSecurityTest() {
        console.log('🧪 Running MesChain Auth Security Test Suite...');
        
        // Test 1: JWT Token Generation
        console.log('\n1️⃣ Testing JWT Token Generation...');
        const testUser = 'testuser@meschain.com';
        const token = this.generateJWTToken(testUser, ['admin', 'super_admin']);
        ...
        const sessionStats = this.getSecurityStats();
        console.log('✅ Session Statistics:', sessionStats);
        
        // Test 5: API Key Generation
        console.log('\n5️⃣ Testing API Key Management...');
        const apiKey = this.generateAPIKey(testUser, ['read', 'write']);
        console.log('✅ API Key Generated:', apiKey);
        
        // Test 6: Security Event Logging
        console.log('\n6️⃣ Testing Security Event Logging...');
        this.logSecurityEvent('test_event', 'Security test completed successfully');
        console.log('✅ Security Events Count:', this.securityEvents.length);
        
        // Test 7: Authentication Flow
        console.log('\n7️⃣ Testing Authentication Flow...');
        const authResult = this.authenticateUser({
            username: 'demo',
            password: 'demo123'
        });
        ...
        this.showNotification(
            ...
            'All 7 security tests passed successfully!', 
            'success'
        );
    ...
    runDiagnostics() {
        console.log('🔍 Running MesChain Auth System Diagnostics...');
        
        const diagnostics = {
            jwtSystem: {
                secretGenerated: !!this.jwtSecret,
                activeTokens: this.activeTokens.size,
            ...
            sessionManagement: {
                ...
                sessionDuration: this.sessionDuration,
                timeRemaining: this.sessionDuration - Math.floor((Date.now() - this.sessionStartTime) / 1000),
...
// 🧪 Global Test Functions for Auth System
function runAuthTests() {
    if (window.mesChainAuth) {
        return window.mesChainAuth.runSecurityTest();
    } else {
        console.error('❌ MesChain Auth System not initialized');
    }
}

function runAuthDiagnostics() {
    if (window.mesChainAuth) {
        return window.mesChainAuth.runDiagnostics();
    } else {
        console.error('❌ MesChain Auth System not initialized');
    }
}
...
function testJWTSystem() {
    if (window.mesChainAuth) {
        ...
        const token = window.mesChainAuth.generateJWTToken('test@meschain.com', ['admin']);
        const validation = window.mesChainAuth.validateJWTToken(token);
...
function showSecurityStats() {
    if (window.mesChainAuth) {
        const stats = window.mesChainAuth.getSecurityStats();
        console.log('📊 Security Statistics:', stats);
        return stats;
    }
}
