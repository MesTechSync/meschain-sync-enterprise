...
class AuthenticationManager {
    ...
    async authenticate(marketplace, credentials) {
        try {
            ...
            let authEndpoint = `${API_ENDPOINTS.BASE_URL}/auth/${marketplace}`;
            let authPayload = credentials;
            
            // Marketplace-specific authentication
            switch(marketplace) {
                case 'trendyol':
                    authPayload = {
    ...
    async refreshToken(marketplace) {
        ...
        try {
            // If no refresh token, re-authenticate with stored credentials
            if (!refreshToken && credentials) {
...
class APIRequestManager {
    ...
    buildHeaders(marketplace, token, customHeaders = {}) {
        ...
        switch(marketplace) {
            ...
            case 'amazon':
                ...
                break;
            case 'ozon':
                baseHeaders['Client-Id'] = this.auth.credentials.get('ozone')?.clientId;
    ...
    async waitForRateLimit(marketplace) {
        const rateLimit = this.rateLimits.get(marketplace);
        const waitTime = Math.min(rateLimit.resetTime - Date.now(), 60000); // Max 1 minute
...
class RealTimeDataManager {
    ...
    async updateOrderMetrics(orderData) {
        // Update order counters in real-time
