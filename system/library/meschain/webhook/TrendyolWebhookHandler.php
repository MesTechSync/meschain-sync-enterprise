...
class TrendyolWebhookHandler {
    ...
    private function processOrderCreated($payload) {
        try {
            $this->registry->get('load')->model('extension/module/meschain_trendyol');
            $model = $this->registry->get('model_extension_module_meschain_trendyol');
            ...
            $orderData = [
                ...
                'order_date' => date('Y-m-d H:i:s', ($payload['orderDate'] ?? time()) / 1000),
    ...
    private function convertToOpenCartOrder($trendyolOrderId, $orderData) {
        try {
            ...
            $this->registry->get('model_extension_module_meschain_trendyol')->linkOrders($trendyolOrderId, $ocOrderId);
