...
class ColumnLeft extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ((string)$this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			// MeChain SYNC Category
			$mechain_sync = [];
			
			if ($this->user->hasPermission('access', 'extension/module/meschain_sync')) {
				$mechain_sync[] = [
					'name'	   => $this->language->get('text_meschain_sync_enterprise'),
			...
			if ($mechain_sync) {
				$marketplace[] = [
					'name'	   => $this->language->get('text_mechain_sync'),
					'href'     => '',
					'children' => $mechain_sync
				];
			}
			...
			// Users - Add directly to system menu instead of as submenu
